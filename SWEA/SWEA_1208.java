...
public class SWEA_1208 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int tc = 1; tc <= 10; tc++) {
			...
			int[] map = new int[100]; // 높이가 저장된 100칸 배열
			int dump = Integer.parseInt(input.readLine()); // 덤프 횟수
			
			String[] s= input.readLine().toString().split(" ");
			...
			int result = Integer.MAX_VALUE; // 높이의 차
			...
			for (int i = 0; i <= dump; i++) {
				...
				result = max - min; // 높이의 차
				if (result == 0|| result ==1) // 높이의 차가 0이면 break
					break;
				else {
					map[maxidx]--;
					map[minidx]++;
