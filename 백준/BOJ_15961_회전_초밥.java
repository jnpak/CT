...
public class BOJ_15961_회전_초밥 {
	...
	public static void main(String[] args) throws IOException {
		...
		D = Integer.parseInt(st.nextToken());	// 초밥의 개수
		K = Integer.parseInt(st.nextToken());	// 연속해서 먹는 접시의 수
		C = Integer.parseInt(st.nextToken()); 	// 서비스 초밥 번호
		...
		System.out.println(converyorbelt());
	}
	private static int converyorbelt() {
		int max =0, kind =0;
		...
		//2. 맨 처음에 먹은 초밥 종류가 최대 종류
		max = kind;
		
		//3. 왼쪽에서부터 하나씩 윈도우에서 제외시키고 오른쪽 초밥을 윈도우에 포함시키면서 이동
		for(int i=1;i<N;i++) {
			//왼쪽 초밥 처리
			...
			//먹은 종류가 많아지거나 같은 경우 쿠폰 반영해서 max값 갱신
			if(max <= kind) {
